class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]: # Monotonic dequeue version. O(n) instead of heap's O(n log k).
    ...
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]: # Proper heap usage, deleting stale values.
        numLength = len(nums)
