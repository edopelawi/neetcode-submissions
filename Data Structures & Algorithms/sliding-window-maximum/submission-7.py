class MonotonicQueue:
    # Short description: This class will drop data that's provably incapable of ever becoming maximum again.
    def __init__(self):
        self.maxq = deque()
...
class Solution:
    def maxSlidingWindow(self, nums: List[int], k: int) -> List[int]: # MonotonicQueue version.
        window = MonotonicQueue() # Not your regular list. MonotonicQueue.
        result = []

        for idx in range(len(nums)):
            if idx < k - 1: # Filling up the first window
                window.push(nums[idx])
            else: # Starting k (end of first window), we go collecting results
                window.push(nums[idx])
                result.append(window.max())
                # Remove the obsolete number for the next window, if it was the max
                oldIndex = idx - k + 1
                window.pop(nums[oldIndex])
        
        return result

    def maxSlidingWindowV5(self, nums: List[int], k: int) -> List[int]: # Non-heap versionm better version of V1.
