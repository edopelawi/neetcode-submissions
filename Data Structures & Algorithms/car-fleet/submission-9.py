class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        # Invariant: after we sort the positions, a different fleet is when a car, based on its initial speed, should reach the destination after the car in front of it. Not on the same time, not before it.
        #
        # or, a better invariant:
        #
        # A new fleet forms when the current car's arrival time is strictly greater than the arrival time of the fleet immediately ahead.

        for pos, time in arrivals:
            if len(fleets) == 0:
                fleets.append(time)
                continue
            
            if time > fleets[-1]: # Arriving later than the front position
                fleets.append(time)
        
        return len(fleets)

    def carFleetV3(self, target: int, position: List[int], speed: List[int]) -> int: # Works, but jumbled code
        # Format: (position, hoursNeeded)
        arrivals = [(pos, (target - pos) / speed[idx]) for idx, pos in enumerate(position)]
        # Sort based on position
        arrivals = sorted(arrivals, key=lambda x: x[0], reverse=True)
